function Basis_Calendar(x, today, calendar_type) {
    ...
    var day = parseInt(today.split("/")[1]) < 10 ? "0" + parseInt(today.split("/")[1]) : parseInt(today.split("/")[1])
    var year = parseInt(today.split("/")[2]) < 10 ? "0" + parseInt(today.split("/")[2]) : parseInt(today.split("/")[2])
    var month = parseInt(today.split("/")[0]) < 10 ? "0" + parseInt(today.split("/")[0]) : parseInt(today.split("/")[0])
    month_dict = {
        "01": "January",
        "02": "February",
        "03": "March",
                ...
                if (month.getAttribute("data-month") == first_month) {
                    if(month.querySelector(".month_change .section_prev_month")){
                        if (window.innerWidth < 1024) {
                            const previousMonth = month.previousElementSibling;
                            if (previousMonth) {
    ...
    if(document.querySelectorAll(".ar_Basis_Date_Box")){
        var ar_len = document.querySelectorAll(".ar_Basis_Date_Box").length;
    }
    if (document.documentElement.clientWidth > 1280) {
        ...
        if(ar_len !== 0){
            ...
            document.querySelector(".Basis_Calendar_Box").classList.add("Basis_Calendar_Box_ar")
        }else {
            document.querySelectorAll(".Basis_Calendar_Box").forEach(function(el) {
    ...
    document.querySelector(".Basis_Calendar_Box").style.transition = "opacity 0.5s";
    document.querySelector(".Basis_Calendar_Box").style.opacity = 0;
    requestAnimationFrame(() => {
    requestAnimationFrame(() => {
        document.querySelector(".Basis_Calendar_Box").style.opacity = 1;
    });
    });
    //
   ...
   const icon = x.closest(".reserve-field").querySelector('.down-icon');
    if(icon){
    ...
    document.querySelectorAll(".Basis_Date").forEach(function (el) {
        if (el.getAttribute("data-active") == "1") {
            if (el.classList.contains("start_date")) {
                document.querySelector(".type_date").textContent = 'departure :';
                const icon = el.closest(".reserve-field").querySelector('.down-icon');
                if(icon && icon.classList.contains('rotate')){
                    icon.classList.remove('rotate');
                }
            } else if (el.classList.contains("end_date")) {
                document.querySelector(".type_date").textContent = 'return :';
                const icon = el.closest(".reserve-field").querySelector('.down-icon');
                if(icon && icon.classList.contains('rotate')){
                    icon.classList.remove('rotate');
                }
            ...
            el.value = x.getAttribute("data-information").split("_")[3];
            el.setAttribute("data-cache", x.getAttribute("data-information"));
            const target = el.nextElementSibling?.nextElementSibling;
            if (target && target.classList.contains("mstring_date")) {
                target.value = x.getAttribute("data-information").split("_")[3];
            }
...
// Set the text content of the next sibling with class "day-of-week"
const nextSibling = el.nextElementSibling;
if (nextSibling && nextSibling.classList.contains("day-of-week")) {
  nextSibling.textContent = weekday;
}
            /*-----------------START JS FOR TOUR LIST JSON TO STYLE CALENDAR---------------------*/
            if(el.closest(".column-date")) {
                el.closest(".column-date").querySelector(".filter-date").setAttribute("data-value", x.getAttribute("data-information").split("_")[3]);
            }
            /*-----------------START JS FOR TOUR LIST JSON TO STYLE CALENDAR---------------------*/
  ...
  document.addEventListener("mouseup", function (e) {
    ...
    if (!container.contains(e.target) && container != e.target) {
        ...
        document.querySelectorAll(".Basis_Date_Box").forEach(box => {
            box.querySelectorAll(".down-icon").forEach(icon => {
            if(icon && icon.classList.contains("rotate")){
                icon.classList.remove("rotate")
            }
            })
  ...
  function close_SimpleCalendar(t){
    document.querySelector(".Basis_Calendar_Box").style.display = "none";
    if(document.querySelector('body').classList.contains("overflow-hidden")){
        document.querySelector('body').classList.remove("overflow-hidden")
    }
