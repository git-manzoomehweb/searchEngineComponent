function Basis_Calendar(x, today) {
    ...
    // update
    document.querySelectorAll(".Basis_Calendar_Box .day").forEach(day => {
        if (!day.classList.contains("empty_day")) {
            const thisDateId = parseInt(day.getAttribute("data-information").split("_")[0]);
    
            if (x.classList.contains("start_date")) {
                if (x.getAttribute("data-type") === "1-6") {
                    if (!x.closest("form").querySelector(".checkin").value) {
                        const form = x.closest("form");
                        const dataCacheStartFlight = form.querySelector(".start_date_flight").getAttribute("data-cache").split("_")[0];
                        const dataCacheDateFlight = form.querySelector(".end_date_flight").getAttribute("data-cache").split("_")[0];
    
                        if (dataCacheStartFlight !== "0" || dataCacheDateFlight !== "0") {
                            if (thisDateId > parseInt(dataCacheDateFlight) || thisDateId < parseInt(dataCacheStartFlight) - 1) {
            ...
            if (x.classList.contains("end_date")) {
                if (x.getAttribute("data-type") === "1-6") {
                    if (!x.closest("form").querySelector(".checkin").value) {
                        ...
                        if (dataCacheStartFlight !== "0" || dataCacheDateFlight !== "0") {
                            if (thisDateId > parseInt(dataCacheDateFlight) || thisDateId < parseInt(dataCacheStartFlight)) {
                                day.removeAttribute("onclick");
                                day.classList.add("disable_day");
                            }
                        }
    ...
    self.querySelectorAll(".year").forEach(function (yearElement) {
        ...
        yearElement.querySelectorAll(".month").forEach(function (monthElement) {
            ...
            monthElement.querySelectorAll(".week").forEach(function (weekElement, week_index) {
                ...
                weekElement.querySelectorAll(".day").forEach(function (dayElement) {
                    if (!dayElement.classList.contains("empty_day") && !dayElement.classList.contains("Special_day")) {
                        ...
                        let this_day = dayElement.getAttribute("data-day");
                        ...
                        if (this_day == today.split("-")[2] && this_month == today.split("-")[1] && this_year == today.split("-")[0]) {
                            ...
                            const dataInformation = dayElement.getAttribute("data-information").split("_")[3];
            ...
            monthElement.querySelector(".month_detail .month_name_gregorian").textContent = month_text;
        });
    });
            ...
            if (yearElement.getAttribute("data-year") == first_year) {
                ...
                yearElement.querySelectorAll(".month").forEach((month, monthIndex) => {
                    if (month.getAttribute("data-month") == first_month) {
                        ...
                        month.querySelector(".month_change .section_prev_month").removeAttribute("onclick");
    ...
    if (calendar_type == 'fa') {
        ...
        enCalendar.style.display = "block";
        faCalendar.style.display = "none";
    }
// update
    if (x.classList.contains("end_date")) {
        if(x.closest(".Basis_Date_Box").querySelector(".start_date").value != ""){
            // is full
            let check_date = x.closest(".Basis_Date_Box").querySelector(".start_date").getAttribute("data-cache").split("_")[0];
        document.querySelectorAll(".Selected_Basis_Calendar_Box .day").forEach(function(day) {
            if (!day.classList.contains("empty_day")) {
...
function Basis_Select_day(x, calendar_type) {
    ...
    document.querySelectorAll(".Basis_Date").forEach(function (dateElement) {
        if (dateElement.getAttribute("data-active") == "1") {
            ...
            if (dateElement.closest(".Basis_Date_Box").classList.contains("format-changed")) {
                ...
                if (calendar_type == 'fa') {
                    if (dateElement.classList.contains("start_date")) {
                        ...
                        const form_action = dateElement.closest('form')?.getAttribute('action');
                        if (/oneway|multi|cip|TouristPanel/i.test(form_action)) {
                            applyDate()
                        }
                    ...
                    } else if (dateElement.classList.contains("end_date") && !dateElement.classList.contains("end_date").disabled) {
                        console.log("ok2")
            ...
            if (calendar_type == 'fa') {
                if(dateElement.closest('.date-container')){
            ...
            if(dateElement.closest(".column-date")){
            ...
            /*-----------------START JS FOR TOUR LIST JSON TO STYLE CALENDAR---------------------*/
            if(dateElement.closest("div").querySelector(".floating-label")) {
            ...
            if (dateElement.classList.contains("start_date")) {
                ...
                // }
                // update
                var check_input_type = dateElement.getAttribute("data-type");
                if (check_input_type !== "1-6") {
                    self.querySelectorAll(".day").forEach(day => {
                        day.classList.remove("selected_date");
        ...
        //here
        document.querySelectorAll(".Basis_Date").forEach(function (dateElement) {
        if (dateElement.classList.contains("start_date") && !dateElement.classList.contains("checkin")) {
            if(dateElement.closest(".Basis_Date_Box").querySelector(".end_date")){
                if (!dateElement.closest(".Basis_Date_Box").querySelector(".end_date").disabled && !dateElement.classList.contains("checkout")) {
                    dateElement.closest(".Basis_Date_Box").querySelector(".end_date").setAttribute("data-active", "1");
                }
            }
        }
...
function calculate_date_diffrent(element) {
    ...
    if (fdate_time < tdate_time) {
        ...
        if(element.closest("form").querySelector("#night-count")){
    ...
    if (window.innerWidth >= 1201) {
        if(!document.querySelector(".tour-list-content")){
                if (change == 0) // Go to the month before
                {
                    ...
                    let nextMonth = x.closest(".month").nextElementSibling;
                    x.closest(".month").classList.remove("current_month");
                    if(nextMonth){
                        x.closest(".month").nextElementSibling.classList.remove("next_current_month");
                    }else if(!nextMonth){
                        const nextYear = x.closest(".month").closest(".year").nextElementSibling;
                        if(nextYear) {
                            nextMonth = nextYear.querySelector(".month").classList.remove("next_current_month");
                        }
                    }
...
function emptySelectedDate(element, calendar_type, type) {
    ...
    if (type == 'dep') {
        ...
        if (activeBtn == 'r-flight') {
            ...
            document.querySelectorAll(`#${activeBtn} #${active_form} .Basis_Date_Box`).forEach(box => { 
        ...
        if(document.querySelector(`#${activeBtn} .start_date`).closest(".date-container")){
        ...
        if(document.querySelector(`#${activeBtn} .end_date`).closest(".date-container")) {
...
function changeCalendar(element, calendar_type) {
    ...
    const activeBtn = document.querySelector('.search-nav ul > .active-module')?.getAttribute('data-id');
    document.querySelectorAll(`#${activeBtn} form`).forEach(form => {
        if (!form.classList.contains("unvisible")) {
            if (form.classList.contains("multicity-flight-form")) {
                ...
                if(form.querySelector('.start_date').closest(".date-container")){
                ...
                if(form.querySelector('.end_date').closest(".date-container")){
